...
export default async function BookingsPage() {
  ...
  const bookings = await prisma.booking.findMany({
    where: { guestId: (session.user as { id: string }).id },
    include: {
      listing: {
        select: {
          id: true,
      ...
      {bookings.length > 0 ? (
        <div className="mt-8 space-y-4">
          {bookings.map((booking) => {
            ...
            return (
              <div
                key={booking.id}
                className="flex flex-col gap-4 rounded-xl border border-gray-200 bg-white p-5 transition-shadow hover:shadow-md sm:flex-row sm:items-center"
              >
                {/* Thumbnail */}
        ...
        <div className="mt-12 flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-gray-300 py-20">
          ...
          <p className="mt-2 text-sm text-gray-500">
            Hãy khám phá và đặt phòng cho chuyến đi tiếp theo
          </p>
          <Link
            href="/listings"
